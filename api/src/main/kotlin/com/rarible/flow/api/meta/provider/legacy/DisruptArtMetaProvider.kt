...
class DisruptArtMetaProvider(
    ...
    override suspend fun getMeta(item: Item): ItemMeta? {
        val itemId = item.id
        if (item.meta.isNullOrBlank()) return emptyMeta(itemId).let {
            logger.warn("DisruptArt::getMeta::Item[$itemId] meta string is empty!")
            it
        }
        val itemMeta = JacksonJsonParser().parseMap(item.meta!!)
        val contentUrl = itemMeta["content"] as String? ?: return emptyMeta(itemId).let {
        ...
        return if (spec.headers.contentType == MediaType.IMAGE_JPEG) {
