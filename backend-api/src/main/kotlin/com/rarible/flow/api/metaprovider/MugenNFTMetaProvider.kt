...
class MugenNFTMetaProvider : ItemMetaProvider {
    ...
    override suspend fun getMeta(item: Item): ItemMeta? {
        val url = "https://onchain.mugenart.io/flow/nft/0x2cd46d41da4ce262/metadata/${item.tokenId}"

        val client = WebClient.create()
        return try {
            val data = client.get().uri(url)
                .retrieve().awaitBodyOrNull<List<MugenNFTMetaBody>>()?.singleOrNull() ?: return null
            ItemMeta(
                itemId = item.id,
                name = data.name,
